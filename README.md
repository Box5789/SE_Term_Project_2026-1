...
#### Windows
PowerShell 또는 명령 프롬프트에서 다음 명령을 사용합니다.
```bat
gradlew.bat run
```
JavaFX UI로 실행하려면 다음 명령을 사용합니다.
```bat
gradlew.bat run --args="javafx"
```
테스트와 전체 빌드는 다음 명령으로 확인합니다.
```bat
gradlew.bat test
...
> Windows에서 실행하려면 Java 17 이상이 설치되어 있고 `JAVA_HOME` 또는 `PATH`에 Java가 설정되어 있어야 합니다.
